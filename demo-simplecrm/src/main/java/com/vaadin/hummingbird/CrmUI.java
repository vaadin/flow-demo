...
public class CrmUI extends UI {
	...
	public class MyNavStateManager implements NavigationStateManager {
		...
		public void setState(String state) {
			mainTemplate.getElement().getNode().enqueueRpc("history.pushState({}, '', $0)", state);
			this.state = state;
		}
	...
	protected void init(VaadinRequest request) {
		...
		Navigator navigator = new Navigator(this, new MyNavStateManager(main), new MyViewDisplay(main));
