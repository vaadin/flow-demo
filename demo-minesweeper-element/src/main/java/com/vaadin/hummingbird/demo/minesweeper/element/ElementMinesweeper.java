...
public class ElementMinesweeper {
    ...
    private void showMessage(String message, String className) {
        Element div = new Element("div").setAttribute("class", className)
                .setProperty("textContent", message);
    ...
    public void reveal(int row, int col) {
        ...
        if (minefield.isMine(row, col)) {
            ...
            if (count > 0) {
                td.setProperty("textContent", count + "");
            } else {
                // Autoreveal
    ...
    private boolean isRevealed(Element td) {
        return td.hasAttribute("class");
    }
