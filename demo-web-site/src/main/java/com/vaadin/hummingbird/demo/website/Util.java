...
public class Util {
    ...
    public static Element createRouterLink(String caption,
            Class<? extends View> targetView) {
        return new Element("a")
                .setAttribute("href", getPath(targetView).orElse(""))
                .setAttribute("routerLink", "").setTextContent(caption);
    }
    ...
    public static Optional<String> getPath(Class<? extends View> childView) {
        return getSiteResolver().getPath(childView);
    }

    public static SiteResolver getSiteResolver() {
        return (SiteResolver) getRouter().getConfiguration().getResolver();
    }
