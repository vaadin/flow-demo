...
public final class MainLayout extends Template {
    ...
    public MainLayout() {
        getModel().importBeans("categories",
                DataService.get().getAllCategories(), property -> true);
    ...
    private void update(LocationChangeEvent locationChangeEvent) {
        ...
        if (categoryId != -1) {
            ...
            getModel().setSelectedCategory(catId);
            getModel().getCategories()
                    .forEach(cat -> onCategorySelect(cat, catId, prodId));
        }
    }
    ...
    private Optional<Element> getElementForCategory(int id) {
        ...
        return ul.get().getChildren().filter(child -> {
            ...
            return Objects.equals(feature.getValue("id"), id);
        }).findFirst();
    }
    ...
    private void onCategorySelect(CategoryModel model, int categoryId,
            int productId) {
        // TODO use ngIf to include the products for expanded category
        Optional<Element> categoryElement = getElementForCategory(
                model.getId());
        if (categoryElement.isPresent()) {
            if (model.getId() == categoryId) {
                expand(model, categoryElement.get());
                selectProduct(model, productId);
            } else {
                collapse(model, categoryElement.get());
            }
        }
    }
    ...
    private void expand(CategoryModel model, Element categoryElement) {
        ...
        String categoryId = getFirstChild(categoryElement, Tag.A).get()
                .as(RouterLink.class).getHref().replace("category/", "");
