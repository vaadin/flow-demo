...
public class Menu {
    ...
    public Menu() {
        ...
        for (Category c : DataService.get().getAllCategories()) {
            ...
            li.getClassList().add(CLASS_COLLAPSED);
            ul.appendChild(li);
        }
    }
    ...
    private void collapse(Element routerLinkElement) {
        Element parent = routerLinkElement.getParent();
        while (parent.getChildCount() > 1) {
            parent.removeChild(1);
        }
        setExpandedClass(routerLinkElement.getParent(), false);

    }

    private static void setExpandedClass(Element e, boolean expanded) {
        e.getClassList().set(CLASS_EXPANDED, expanded);
        e.getClassList().set(CLASS_COLLAPSED, !expanded);
    }

    private void expand(Element routerLinkElement) {
        if (isExpanded(routerLinkElement)) {
            return;
        }
        productLinks.clear();

        setExpandedClass(routerLinkElement.getParent(), true);
    ...
    private void select(Element e) {
        e.getClassList().add(CLASS_SELECTED);
    }

    private void deselect(Element e) {
        e.getClassList().remove(CLASS_SELECTED);
    }
